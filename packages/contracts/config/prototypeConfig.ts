...
const percentsToThresholds = (percents: { none: number; retreat: number; lateral: number; expand: number }) => {
  if (percents.none + percents.retreat + percents.lateral + percents.expand > 1)
    throw new Error("percents must sum to 1");
  const none = percents.none;
  const retreat = percents.retreat + none;
  const lateral = percents.lateral + retreat;
  const expand = percents.expand + lateral;
