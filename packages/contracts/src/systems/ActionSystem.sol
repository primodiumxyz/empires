...
contract ActionSystem is EmpiresSystem {
    ...
    function chargeShield(bytes32 _planetId) public payable _onlyNotGameOver _takeRake {
        ...
        require(planetData.factionId != EEmpire.NULL, "[ActionSystem] Planet is not owned");
        require(
            _msgValue() == LibPrice.getTotalCost(EPlayerAction.ChargeShield, planetData.factionId, true),
            "[ActionSystem] Incorrect payment"
        );

        _purchaseAction(EPlayerAction.ChargeShield, planetData.factionId, true, _msgValue());
    ...
    function drainDestroyer(bytes32 _planetId) public payable _onlyNotGameOver _takeRake {
        ...
        require(planetData.factionId != EEmpire.NULL, "[ActionSystem] Planet is not owned");
        require(
            _msgValue() == LibPrice.getTotalCost(EPlayerAction.DrainShield, planetData.factionId, false),
            "[ActionSystem] Incorrect payment"
        );

        _purchaseAction(EPlayerAction.DrainShield, planetData.factionId, false, _msgValue());
     ...
     */
    function _purchaseAction(EPlayerAction _actionType, EEmpire _empireImpacted, bool _progressAction, uint256 _spend)
        private
    {
