...
import { PayoutManager } from "../../test/mocks/PayoutManager.sol";

contract DeployPayoutManager is Script {
  string PAYMAN_PATH = "./payman.json";
  ...
  function run() external returns (PayoutManager) {
    ...
    if (getPaymanAddress() == address(0)) {
      ...
      string memory output = vm.serializeAddress(obj2, "ADDRESS", address(payman));
