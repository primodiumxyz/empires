...
contract ActionSystemTest is PrimodiumTest {
    ...
    function testChargeShield() public {
        ...
        uint256 cost = LibPrice.getTotalCost(EPlayerAction.ChargeShield, Planet.getFactionId(planetId), true);
    ...
    function testDrainShield() public {
        ...
        uint256 currentShields = Planet.get(planetId).shieldCount;
        if (currentShields == 0) {
            cost = LibPrice.getTotalCost(EPlayerAction.ChargeShield, Planet.getFactionId(planetId), true);
            world.Empires__chargeShield{value: cost}(planetId);
            currentShields = 1;
            assertEq(Planet.get(planetId).shieldCount, currentShields);
        }

        cost = LibPrice.getTotalCost(EPlayerAction.DrainShield, Planet.getFactionId(planetId), false);
    ...
    function testPurchaseActionRegress() public {
        ...
        assertEq(
            Balances.get(EMPIRES_NAMESPACE_ID), initBalance + totalCost, "Namespace should have received the balance"
        );
        assertEq(
            PointsMap.get(EEmpire.Blue, bobId), P_PointConfig.getPointUnit(), "Player should have received blue points"
        );
        assertEq(
