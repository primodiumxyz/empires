...
export const OverrideButton: React.FC<{
  ...
  stroke?: string;
  fill?: string;
}> = ({ index, icon, stroke = "#6EEDFF", fill = "#2C4148" }) => {
  const currentIndex = useTabs();
  return (
    <Tabs.Button variant="ghost" size="content" index={index} className={cn(currentIndex === index && "scale-110")}>
      <svg width="78" height="86" viewBox="0 0 78 86" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          ...
          stroke={stroke}
        />
        <path d="M38.6422 4L72.4172 23.5M38.6422 82L4.86719 62.5" stroke={stroke} stroke-dasharray="2 2" />
        <path
